...
// Use the long-term capital gains brackets from federal.ts
// so they stay in sync with the tax year constants.
const cutoffAmounts: Cutoffs = Object.fromEntries(
  Object.values(FilingStatus).map((fs) => [
    fs,
    federalBrackets.longTermCapGains.status[fs as FilingStatus].brackets
  ])
) as Cutoffs
