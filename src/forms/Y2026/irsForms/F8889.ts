...
export default class F8889 extends F1040Attachment {
  ...
  */
  ...
  // Line 7: Additional contribution amount for age 55+
  // $1,000 catch-up if the person was 55 or older at end of tax year
  l7 = (): number | undefined => {
    ...
    const age = endOfYear.getFullYear() - dobDate.getFullYear() -
      (endOfYear < new Date(endOfYear.getFullYear(), dobDate.getMonth(), dobDate.getDate()) ? 1 : 0)
  ...
  // Line 17a: Exception to 20% additional tax applies if the person
  // was age 65 or older at end of tax year OR became disabled.
  l17a = (): boolean => {
    const dob = this.person.dateOfBirth
    ...
    const age =
      ...
      dobDate.getFullYear() -
      (endOfYear <
      new Date(endOfYear.getFullYear(), dobDate.getMonth(), dobDate.getDate())
        ? 1
        : 0)
    return age >= 65
  }
  // Line 17b: Additional tax on HSA distributions (20% unless exempt)
