...
export default class F8960 extends F1040Attachment {
  ...
  */
  // Annuity income subject to NIIT: taxable amount from 1099-R with
  // distribution code "D" in box 7 (annuity not from qualified plan)
  l3 = (): number | undefined => {
    const annuities = this.f1040
      .f1099rs()
      .filter((f) => f.form.distributionCode?.includes('D'))
    if (annuities.length === 0) return undefined
    return annuities.reduce((sum, f) => sum + f.form.taxableAmount, 0)
  }
  ...
  */
  ...
  // MAGI for NIIT = AGI + foreign earned income exclusion (Form 2555 line 50)
  l13 = (): number =>
    this.f1040.l11() + (this.f1040.f2555?.l50() ?? 0)
