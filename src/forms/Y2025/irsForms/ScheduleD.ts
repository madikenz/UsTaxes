...
export default class ScheduleD extends F1040Attachment {
  ...
  constructor(f1040: F1040) {
    ...
    this.rateGainWorksheet = new SDRateGainWorksheet(f1040)
  ...
  // Line 5: Short-term gain/loss from Schedule K-1 (partnerships, S corps, etc.)
  l5 = (): number | undefined => {
    const total = this.f1040.info.scheduleK1Form1065s.reduce(
      (sum, k1) => sum + (k1.shortTermCapitalGains ?? 0),
      0
    )
  ...
  l6 = (): number | undefined => {
    ...
    if (carryover !== undefined && carryover > 0) {
      return -carryover // Entered as negative on the form
    }
    return undefined
  }
  ...
  // Line 12: Net long-term gain/loss from Schedule K-1
  l12 = (): number | undefined => {
    const total = this.f1040.info.scheduleK1Form1065s.reduce(
      (sum, k1) => sum + (k1.longTermCapitalGains ?? 0),
      0
    )
    return total !== 0 ? total : undefined
  }
