...
export default class F6251 extends F1040Attachment {
  ...
  // If taxpayer elected to include cap gains in investment income (Form 4952 l4g),
  // that election doesn't apply for AMT. The AMT deduction may be lower.
  l2c = (): number | undefined => {
    const f4952 = this.f1040.f4952
    if (f4952 === undefined) return undefined
    const electedAmount = f4952.l4g() ?? 0
    if (electedAmount <= 0) return undefined
    // Regular tax deduction (Form 4952, line 8)
    const regularDeduction = f4952.l8() ?? 0
    // AMT: recompute without the l4g election
    // AMT investment income = l4c + l4f (no l4g)
    const amtInvestmentIncome = f4952.l4c() + f4952.l4f()
    const amtNetInvestmentIncome = Math.max(0, amtInvestmentIncome - f4952.l5())
    const amtDeduction = Math.min(f4952.l3(), amtNetInvestmentIncome)
    const diff = regularDeduction - amtDeduction
    return diff > 0 ? diff : undefined
  }

  // Depletion (difference between regular tax and AMT).
  // Requires parallel AMT depletion computation — not available without
  // separate cost/percentage depletion data model.
