...
export default class ScheduleEIC extends F1040Attachment {
  ...
  // Under 24 at end of year (full-time student): born CURRENT_YEAR - 23 or later
  qualifyingStudentCutoffYear = CURRENT_YEAR - 23
  // Under 19 at end of year: born CURRENT_YEAR - 18 or later
  qualifyingCutoffYear = CURRENT_YEAR - 18
  ...
  // step 2, question 1
  // Per Pub 596: investment income = taxable interest + tax-exempt interest
  //   + ordinary dividends + capital gains + net rental/royalty income (if positive)
  investmentIncome = (): number =>
    sumFields([
      ...
      Math.max(this.f1040.schedule1.l5() ?? 0, 0)
    ])
  ...
  // 5.1.2 - Taxable scholarship/grant income not on W-2
  taxableScholarshipIncome = (): number =>
    this.f1040.info.scholarshipIncome ?? 0
  ...
  // 5.2 - Self-employed if Schedule C or F filed
